...
<body>
	...
	<script>
		...
		//비밀 번호 유효성 여부를 관리할 변수 만들고 초기값 부여
		let isPwdValid=false;
		
		//닉네임과 비밀번호의 유효성 여부를 이용해서 버튼에 disabled 속성을 추가 혹은 제거하는 함수
		const checkForm = ()=>{
			//만일 닉네임도 유효 하고 그리고 비밀번호도 유효하다면
		...
		document.querySelector("#nick").addEventListener("input", (e)=>{
			...
			.then(data=>{
				...
				if(data.canUse){
					...
					e.target.classList.add("is-invalid");//is-invalid 클래스를 추가
					isNickValid=false;
				} 
				//위에서 변경한 isNickValid 값이 button 에 반영되도록 함수를 호출해준다.
				checkForm();
			});
		});
		...
		const checkPwd = ()=>{
			...
			if(pwd == pwd2){
				...
				isPwdValid=false;
			}//위에서 변경한 isPwdValid 값이 button 에 반영되도록 함수를 호출해준다.
			checkForm();
		}
